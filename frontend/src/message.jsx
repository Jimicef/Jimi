      ...
      >
        {message.system ?(
          ...
          >
            ...
            {message.textArray ?(
              <Box>
                {message.textArray.map((supportItem, idx) => (
                  <Box>
                    <Typography variant="body1" sx={{wordBreak: 'break-word', fontWeight: '460'}}>{idx+1}번: {supportItem}</Typography>
                  </Box>
                ))}
              </Box>
            ):(
              ...
              <Box sx={{width: '95%'}}> 
                ...
                {(message.support) ? (
                    ...
                    <Box sx={{width: ['90%', '90%', '93%']}}>
                      ...
                      <br/>
                      <Typography variant="body2" onClick={()=>handleQuestion("이 서비스를 신청하려면 어떤 서류가 필요하나요?")} sx={{"&:hover": {color:"grey.500", cursor: 'pointer'}, display: "inline-block", alignItems:'center', color:"#1A66CC"}}><SendIcon sx={{fontSize: "14px", color: "#1A66CC"}}/>
                      이 서비스를 신청하려면 어떤 서류가 필요하나요?        </Typography>
                      <br/>
